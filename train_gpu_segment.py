...
segment_path_dir = './mvit/DATA/01_lesions_masks'
...
class MViT(nn.Module):
    ...
    def forward(self, x, cnn_output):
        ...
        # reduce down to (batch_size, 768)
        x = torch.mean(x,1)

        # concat cnn
        x = torch.cat([x,cnn_output],1)
        x = self.norm(x)
        x = self.head(x)
        return x
...
# load cnn object in your checkpoints if it exists
try:
    mycnn = torch.load('./checkpoints/cnn_model.pt')
    print('loaded previous cnn model')
except:
    print('using new cnn')
    pass
...
class Imagenet(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        self.cfg = cfg
        self.mode = mode
        self.data_path = cfg.DATA.PATH_TO_DATA_DIR # path to image folders 
        assert mode in [
            "train",
            "val",
            "test",
        ...
        if cfg.DATA.PATH_TO_PRELOAD_IMDB == "":
            self._construct_imdb() # this is the one used

    def _construct_imdb(self):
        """Constructs the imdb."""
        # Compile the split data path
        split_path = os.path.join(self.data_path, self.mode)
        # add another split path
        split_path_seg = os.path.join(segment_path_dir,self.mode) # adding in separate path for segmentation images 
        ...
        split_files_seg = pathmgr.ls(split_path_seg)

        # Map ImageNet class ids to contiguous ids
        self._class_id_cont_id = {v: i for i, v in enumerate(self._class_ids)}

        # need to ensure that the images are ordered the same way
        ...
        for class_id in self._class_ids:
            ...
            im_dir_seg = os.path.join(split_path_seg, class_id)
            for im_name in pathmgr.ls(im_dir):
                if (os.path.exists(os.path.join(im_dir,im_name))) & (os.path.exists(os.path.join(im_dir_seg,im_name))):
                    im_path = os.path.join(im_dir, im_name)
                    im_path_seg = os.path.join(im_dir_seg, im_name)
                    self._imdb.append({"im_path": [im_path, im_path_seg], "class": cont_id})
                else:
                    continue
    ...
    def _prepare_im_seg(self, im_path_seg):
        im = Image.open(im_path_seg)
    ...
    def __load__(self, index):
        try:
            # Load the image
            im_path_actual = self._imdb[index]["im_path"][0] # load the actual image
            im_path_seg = self._imdb[index]["im_path"][1] # load the mask
            # Prepare the image for training / testing
            if self.mode == "train" and self.cfg.AUG.NUM_SAMPLE > 1:
                im = []
                for _ in range(self.cfg.AUG.NUM_SAMPLE):
                    crop = self._prepare_im(im_path_actual)
                    im.append(crop)
                    im_seg = self._prepare_im_seg(im_path_seg)
                return [im, im_seg]
            else:
                im = self._prepare_im(im_path_actual)
                im_seg = self._prepare_im_seg(im_path_seg)
                return [im, im_seg]
    ...
    def __getitem__(self, index):
        ...
        return im, label # im is now a list [image, image_seg] so the input has to be separated within the data loader
...
def train_epoch(
    ...
    f1_holder = []
    top1_err_holder=[]
    for cur_iter, ([inputs,inputs_seg], [labels,labels_seg]) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            cnn_output = cnn_model(inputs_seg)
            preds = model(inputs, cnn_output)
            #testlist2.append(preds)
            #testlist2.append(labels)
...
def train(cfg):
    ...
    for cur_epoch in range(start_epoch+1, cfg.SOLVER.MAX_EPOCH):
        ...
        if len(dom_epoch_wise["f1"]) == 0 or np.nanmean(f1) > max(dom_epoch_wise["f1"]):
            ...
            # LF - save cnn model if f1 performs better
            torch.save(cnn_model,'./checkpoints/cnn_model.pt')
        ...
        if is_eval_epoch:
            eval_f1,eval_err = eval_epoch(val_loader, model, cnn_model, val_meter, cur_epoch, cfg)
...
@torch.no_grad()
def eval_epoch(val_loader, model, cnn_model, val_meter, cur_epoch, cfg):
    """
    ...
    f1_holder = []
    top1_err_holder = []
    for cur_iter, ([inputs,inputs_seg], [labels,labels_seg]) in enumerate(val_loader):
        if cfg.NUM_GPUS:
