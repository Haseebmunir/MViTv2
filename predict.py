...
import configparser
import shutil
from evaluate import getPerformance, predict_val
...
# Minjun Jung: ***************
# In order to use label-wise test prediction & stat use below function.
def report_performance(loader, curr_model, cfg):
    y_true, y_pred = predict_val(loader, curr_model, cfg)
    label_lookup = {v: k for k, v in loader.dataset._class_id_cont_id.items()}
    getPerformance(y_true, y_pred, label_lookup, title="Confusion Matrix", dir=new_dir)
    filenames = list(map(lambda x: x['im_path'], loader.dataset._imdb))
    predictions = list(map(lambda x: label_lookup[x], y_pred))
    return list(zip(filenames, predictions))
# ===============
...
if __name__ == '__main__':
    ...
    new_dir = "./logs/[PRED] " + datetime.now().strftime("%Y_%m_%d_%H%M%S")
    os.mkdir(new_dir)
    shutil.copyfile(src=config_path, dst=new_dir + "/" + config_path.split("/")[-1])
    setupLogging(new_dir)

    start = time.time()
    preds = report_performance(test_loader, curr_model, cfg)

    # Save the predictions as its own CSV
    with open(new_dir + "/predictions.csv", "w") as file:
        file.write("file_name,diagnosis\n")
